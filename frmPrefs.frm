...
Begin VB.Form planetPrefs 
   ...
   Begin VB.Frame fraDevelopment 
      ...
      Begin VB.Frame fraDevelopmentInner 
         ...
         Begin VB.ComboBox cmbDebug 
            Height          =   315
            ItemData        =   "frmPrefs.frx":2795
            Left            =   1530
            List            =   "frmPrefs.frx":2797
   ...
   Begin VB.Frame fraConfig 
      ...
      Begin VB.Frame fraConfigInner 
         ...
         Begin VB.ComboBox cmbScrollWheelDirection 
            Height          =   315
            ItemData        =   "frmPrefs.frx":4339
            Left            =   1995
            List            =   "frmPrefs.frx":433B
            Style           =   2  'Dropdown List
            TabIndex        =   90
   ...
   Begin VB.Frame fraPosition 
      ...
      Begin VB.Frame fraPositionInner 
         ...
         End
         Begin VB.Label lblPosition 
            Caption         =   $"frmPrefs.frx":874C
            Height          =   3045
            Index           =   6
            Left            =   2265
            TabIndex        =   68
            Tag             =   "lblAlarmSoundDesc"
            ToolTipText     =   $"frmPrefs.frx":88E8
            Top             =   450
            Width           =   5175
         End
         Begin VB.Label lblPosition 
...
Private gblPlPrefsLoadedFlg As Boolean
...
Private Sub Form_Load()
    ...
    gblPlFormXPosTwips = ""
    gblPlFormYPosTwips = ""
    gblPlPrefsLoadedFlg = True ' this is a variable tested by an added form property to indicate whether the form is loaded or not
...
Private Sub chkEnableBalloonTooltips_Click()
   On Error GoTo chkEnableBalloonTooltips_Click_Error
    ...
    If chkEnableBalloonTooltips.Value = 1 Then
        gblPlEnableBalloonTooltips = "1"
    Else
        gblPlEnableBalloonTooltips = "0"
    End If
...
Private Sub showLastTab()
   ...
   On Error GoTo showLastTab_Error
    ...
    If gblPlLastSelectedTab = "general" Then Call picButtonMouseUpEvent("general", imgGeneral, imgGeneralClicked, fraGeneral, fraGeneralButton)  ' was imgGeneralMouseUpEvent
    If gblPlLastSelectedTab = "config" Then Call picButtonMouseUpEvent("config", imgConfig, imgConfigClicked, fraConfig, fraConfigButton)     ' was imgConfigMouseUpEvent
    If gblPlLastSelectedTab = "position" Then Call picButtonMouseUpEvent("position", imgPosition, imgPositionClicked, fraPosition, fraPositionButton)
    If gblPlLastSelectedTab = "development" Then Call picButtonMouseUpEvent("development", imgDevelopment, imgDevelopmentClicked, fraDevelopment, fraDevelopmentButton)
    If gblPlLastSelectedTab = "fonts" Then Call picButtonMouseUpEvent("fonts", imgFonts, imgFontsClicked, fraFonts, fraFontsButton)
    If gblPlLastSelectedTab = "sounds" Then Call picButtonMouseUpEvent("sounds", imgSounds, imgSoundsClicked, fraSounds, fraSoundsButton)
    If gblPlLastSelectedTab = "window" Then Call picButtonMouseUpEvent("window", imgWindow, imgWindowClicked, fraWindow, fraWindowButton)
    If gblPlLastSelectedTab = "about" Then Call picButtonMouseUpEvent("about", imgAbout, imgAboutClicked, fraAbout, fraAboutButton)
...
'    Dim thisText As String: thisText = vbNullString
    ...
    ' config
    gblPlEnableTooltips = LTrim$(Str$(chkEnableTooltips.Value))
    gblPlEnableBalloonTooltips = LTrim$(Str$(chkEnableBalloonTooltips.Value))
    gblPlShowTaskbar = LTrim$(Str$(chkShowTaskbar.Value))
    
    gblPlGaugeSize = LTrim$(Str$(sliGaugeSize.Value))
    gblPlScrollWheelDirection = LTrim$(cmbScrollWheelDirection.ListIndex)
    
    ' general
    gblPlGaugeFunctions = LTrim$(Str$(chkGaugeFunctions.Value))
    
    ''PrWidgetSkew = LTrim$(Str$('sliWidgetSkew.Value))
    gblPlStartup = LTrim$(Str$(chkGenStartup.Value))
    
    ' sounds
    gblPlEnableSounds = LTrim$(Str$(chkEnableSounds.Value))
    
    'development
    gblPlDebug = LTrim$(Str$(cmbDebug.ListIndex))
    gblPlDblClickCommand = txtDblClickCommand.Text
    gblPlOpenFile = txtOpenFile.Text
    gblPlDefaultEditor = txtDefaultEditor.Text
    
    ' position
    gblPlAspectHidden = LTrim$(Str$(cmbAspectHidden.ListIndex))
    gblPlWidgetPosition = LTrim$(Str$(cmbWidgetPosition.ListIndex))
    gblPlWidgetLandscape = LTrim$(Str$(cmbWidgetLandscape.ListIndex))
    gblPlWidgetPortrait = LTrim$(Str$(cmbWidgetPortrait.ListIndex))
    gblPlLandscapeFormHoffset = txtLandscapeHoffset.Text
    gblPlLandscapeFormVoffset = txtLandscapeVoffset.Text
    gblPlPortraitHoffset = txtPortraitHoffset.Text
    gblPlPortraitYoffset = txtPortraitYoffset.Text
    
'    gblPlvLocationPercPrefValue
'    gblPlhLocationPercPrefValue

    ' fonts
    gblPlPrefsFont = txtPrefsFont.Text
    gblPlPrefsFontSize = txtPrefsFontSize.Text
    'PrPrefsFontItalics = txtFontSize.Text

    ' Windows
    gblPlWindowLevel = LTrim$(Str$(cmbWindowLevel.ListIndex))
    gblPlPreventDragging = LTrim$(Str$(chkPreventDragging.Value))
    gblPlOpacity = LTrim$(Str$(sliOpacity.Value))
    gblPlWidgetHidden = LTrim$(Str$(chkWidgetHidden.Value))
    gblPlHidingTime = LTrim$(Str$(cmbHidingTime.ListIndex))
    gblPlIgnoreMouse = LTrim$(Str$(chkIgnoreMouse.Value))
    ...
    If gblPlStartup = "1" Then
        Call savestring(HKEY_CURRENT_USER, "SOFTWARE\Microsoft\Windows\CurrentVersion\Run", "EuropaMarbleWidget", """" & App.Path & "\" & "europa.exe""")
    Else
        Call savestring(HKEY_CURRENT_USER, "SOFTWARE\Microsoft\Windows\CurrentVersion\Run", "EuropaMarbleWidget", vbNullString)
    End If
...
Private Sub btnPrefsFont_Click()
    ...
    btnSave.Enabled = True ' enable the save button
    fntFont = gblPlPrefsFont
    fntSize = Val(gblPlPrefsFontSize)
    ...
    gblPlPrefsFontItalics = CStr(fntItalics)
...
Private Sub adjustPrefsControls()
    ...
    ' general tab
    chkGaugeFunctions.Value = Val(gblPlGaugeFunctions)
    chkGenStartup.Value = Val(gblPlStartup)
    ...
    sliGaugeSize.Value = Val(gblPlGaugeSize)
    If sliGaugeSize.Value <> sliGaugeSizeOldValue Then
        btnSave.Visible = True
    End If
    
    cmbScrollWheelDirection.ListIndex = Val(gblPlScrollWheelDirection)
    chkEnableTooltips.Value = Val(gblPlEnableTooltips)
    chkEnableBalloonTooltips.Value = Val(gblPlEnableBalloonTooltips)
    chkShowTaskbar.Value = Val(gblPlShowTaskbar)
    
    
    ' sounds tab
    chkEnableSounds.Value = Val(gblPlEnableSounds)
    
    ' development
    cmbDebug.ListIndex = Val(gblPlDebug)
    txtDblClickCommand.Text = gblPlDblClickCommand
    txtOpenFile.Text = gblPlOpenFile
    txtDefaultEditor.Text = gblPlDefaultEditor
    ...
    ' fonts tab
    txtPrefsFont.Text = gblPlPrefsFont
    txtPrefsFontSize.Text = gblPlPrefsFontSize
    
    ' position tab
    cmbAspectHidden.ListIndex = Val(gblPlAspectHidden)
    cmbWidgetPosition.ListIndex = Val(gblPlWidgetPosition)
    ...
    'cmbWidgetLandscape
    cmbWidgetLandscape.ListIndex = Val(gblPlWidgetLandscape)
    cmbWidgetPortrait.ListIndex = Val(gblPlWidgetPortrait)
    txtLandscapeHoffset.Text = gblPlLandscapeFormHoffset
    txtLandscapeVoffset.Text = gblPlLandscapeFormVoffset
    txtPortraitHoffset.Text = gblPlPortraitHoffset
    txtPortraitYoffset.Text = gblPlPortraitYoffset

    ' Windows tab
    cmbWindowLevel.ListIndex = Val(gblPlWindowLevel)
    chkIgnoreMouse.Value = Val(gblPlIgnoreMouse)
    chkPreventDragging.Value = Val(gblPlPreventDragging)
    sliOpacity.Value = Val(gblPlOpacity)
    chkWidgetHidden.Value = Val(gblPlWidgetHidden)
    cmbHidingTime.ListIndex = Val(gblPlHidingTime)
...
Private Sub fraAbout_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    fraScrollbarCover.Visible = True
    If gblPlEnableTooltips = "1" Then CreateToolTip fraAbout.hwnd, "The About tab tells you all about this program and its creation using VB6.", _
...
Private Sub fraDevelopment_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If gblPlEnableBalloonTooltips = "1" Then CreateToolTip fraDevelopment.hwnd, "This tab contains elements that will assist in debugging and developing this program further. ", _
...
Private Sub fraDevelopmentInner_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If gblPlEnableBalloonTooltips = "1" Then CreateToolTip fraDevelopmentInner.hwnd, "This tab contains elements that will assist in debugging and developing this program further. ", _
                  TTIconInfo, "Help on the Development Tab", , , , True

End Sub
Private Sub fraFonts_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If gblPlEnableBalloonTooltips = "1" Then CreateToolTip fraFonts.hwnd, "This tab allows you to set a specific font for the preferences only as there are no textual elements in the main program. We suggest Centurion Light SF at 8pt, which you will find bundled in the gblPl program folder. Choose a small 8pt font for each.", _
...
Private Sub fraFontsInner_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If gblPlEnableBalloonTooltips = "1" Then CreateToolTip fraFontsInner.hwnd, "This tab allows you to set a specific font for the preferences only as there are no textual elements in the main program. We suggest Centurion Light SF at 8pt, which you will find bundled in the gblPl program folder. Choose a small 8pt font for each.", _
                  TTIconInfo, "Help on Setting the Fonts", , , , True
End Sub
...
Private Sub fraGeneral_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If gblPlEnableBalloonTooltips = "1" Then CreateToolTip fraGeneral.hwnd, "The General Panel contains the most important user-configurable items required for the program to operate correctly.", _
...
Private Sub fraGeneralInner_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If gblPlEnableBalloonTooltips = "1" Then CreateToolTip fraGeneralInner.hwnd, "The General Panel contains the most important user-configurable items required for the program to operate correctly.", _
                  TTIconInfo, "Help on Essential Configuration", , , , True
End Sub

Private Sub fraPosition_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
     If gblPlEnableTooltips = "1" Then CreateToolTip fraPosition.hwnd, "This tab allows you to determine the X and Y positioning of your widget in landscape and portrait screen modes. Best left well alone unless you use Windows on a tablet.", _
...
Private Sub fraPositionInner_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If gblPlEnableBalloonTooltips = "1" Then CreateToolTip fraPositionInner.hwnd, "This tab allows you to determine the X and Y positioning of your widget in landscape and portrait screen modes. Best left well alone unless you use Windows on a tablet.", _
                  TTIconInfo, "Help on Tablet Positioning", , , , True
End Sub
...
Private Sub fraSounds_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
 If gblPlEnableTooltips = "1" Then CreateToolTip fraSounds.hwnd, "The sound panel allows you to configure the sounds that occur within gblPl. Some of the animations have associated sounds, you can control these here..", _
...
Private Sub fraSoundsInner_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
     If gblPlEnableTooltips = "1" Then CreateToolTip fraSoundsInner.hwnd, "The sound panel allows you to configure the sounds that occur within gblPl. Some of the animations have associated sounds, you can control these here..", _
                  TTIconInfo, "Help on Configuring Sounds", , , , True
End Sub
...
Private Sub fraWindowInner_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
     If gblPlEnableTooltips = "1" Then CreateToolTip fraWindowInner.hwnd, "The Opacity and Window Level of the program are rather strange characteristics to change in a Windows program, however this widget is a copy of a Yahoo Widget of the same name. All widgets have similar window tab options including the capability to change the opacity and window level. Whether these options are useful to you or anyone is a moot point but as this tool aims to replicate the YWE version functionality it has been reproduced here. It is here as more of an experiment as to how to implement a feature, one carried over from the Yahoo Widget (javascript) version of this program.", _
                  TTIconInfo, "Help on YWE Quirk Mode Options", , , , True
End Sub
...
Private Sub lblGitHub_dblClick()
    ...
    If answer = vbYes Then
       Call ShellExecute(Me.hwnd, "Open", "https://github.com/yereverluvinunclebert/Europa-VB6-Widget", vbNullString, App.Path, 1)
    End If
End Sub
...
Private Sub adjustPrefsTheme()
   On Error GoTo adjustPrefsTheme_Error
    ...
    If gblPlSkinTheme <> vbNullString Then
        If gblPlSkinTheme = "dark" Then
            Call setThemeShade(212, 208, 199)
        Else
            Call setThemeShade(240, 240, 240)
        End If
...
Private Sub chkEnableResizing_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If gblPlEnableTooltips = "1" Then CreateToolTip chkEnableResizing.hwnd, "This allows you to resize the whole prefs window by dragging the bottom right corner of the window. It provides an alternative method of supporting high DPI screens.", _
